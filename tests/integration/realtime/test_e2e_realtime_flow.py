...
def _try_create_engine(engine_type: str = "whispers2t_base", device: str = "cpu"):
...
class TestSileroVADDirect:
    ...
    def test_silero_vad_detects_speech(self, audio_file_en: Path):
        ...
        # Load audio file
        with FileSource(str(audio_file_en), chunk_duration_ms=32) as source:
            probabilities = []
            for chunk in source:
                if len(chunk) >= 512:
...
class TestStreamTranscriberE2E:
    ...
    def test_stream_transcriber_e2e_flow_en(self, audio_file_en: Path):
        ...
        engine = _try_create_engine("whispers2t_base", "cpu")
        if engine is None:
    ...
    def test_stream_transcriber_e2e_flow_ja(self, audio_file_ja: Path):
        ...
        # Try to create engine (WhisperS2T supports Japanese)
        engine = _try_create_engine("whispers2t_base", "cpu")
        if engine is None:
            _skip_or_fail("WhisperS2T engine could not be initialized")
